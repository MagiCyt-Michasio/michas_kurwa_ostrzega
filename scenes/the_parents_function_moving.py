...
class TheParentsFunctionMoving(MovingCameraScene):
    def construct(self):
        # Kolory
        ...
        def child_y(x):
            # Przykład: lekko opadająca, lekko falująca
            return -0.15 * x + 0.4 * np.sin(0.7 * x)

        def parent_y(x):
            # Trochę wyżej, też lekko falująca
        ...
        # Etykiety podążające za czołem linii
        ...
        # Kamera podąża za wspólnym obszarem Child i Parent
        def update_camera(mob):
            ct = t.get_value()
            cy = child_y(ct)
            py = parent_y(ct)
            y_center = (cy + py) / 2
            mob.move_to([ct + 0.5, y_center, 0])

        self.camera.frame.add_updater(update_camera)

        # Animacja: t rośnie płynnie od 0 do 12
